...
class Bias(Layer):
    ...
    def call(self, inputs):
        outputs = K.bias_add(inputs, self.bias)
        return outputs
...
def partial_convolution(input_, mask, filters, shape, stride, activation):
    convolution_layer = Conv1D(
        filters, shape, strides=stride, use_bias=False, padding="same"
    )

    mask_sum_layer = Conv1D(
        filters,
        shape,
        strides=stride,
        padding="same",
        weights=[np.ones((shape[0], input_.shape[-1], filters)), np.zeros((filters,))],
    )
    ...
    inv_sum = Lambda(lambda x: filters * shape[0] / (0.0001 + x))(mask_sum)
...
def normal_convolution(input_, mask, filters, shape, stride, activation):
    convolution_layer = Conv1D(filters, shape, strides=stride, padding="same")
...
def residual_block(input_, mask):
    output, new_mask = normal_convolution(
        input_, mask, 512, (3,), 1, Activation("relu")
    )
    output, new_mask = normal_convolution(
        output, new_mask, 512, (3,), 1, Activation("relu")
    )
    output, new_mask = normal_convolution(
        output, new_mask, int(input_.shape[-1]), (3,), 1, Activation("relu")
    )
...
def sequence_unet(patch_size=128, input_dim=37, output_dim=33):
    ...
    output = input_
    mask = input_mask
    for shape, filters in zip(
        [7, 5, 5, 3, 3, 3, 3], [64, 128, 256, 256, 256, 256, 256]
    ):
        skips.append((output, mask))
        print(output.shape)
        output, mask = partial_convolution(
            output, mask, filters, (shape,), 2, Activation("relu")
        )
        if shape != 7:
            output = BatchNormalization()(output)
    for shape, filters in zip(
        [4, 4, 4, 4, 4, 4, 4], [256, 256, 256, 256, 128, 64, output_dim]
    ):
        ...
        if filters != output_dim:
            activation = tensorflow.keras.layers.LeakyReLU(0.2)
        else:
            activation = Activation("linear")
        output, mask = partial_convolution(
            output, mask, filters, (shape,), 1, activation
        )
