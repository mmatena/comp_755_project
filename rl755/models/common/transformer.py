...
class AutoregressiveTransformer(tf.keras.Model):
    ...
    def build(self, input_shape):
        ...
        self.initial_layer.build(input_shape=input_shape)
        ...
        self.transformer.build(input_shape=transformer_input_shape)
        self.final_layer = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Dense(units=self.output_size, activation=None)
        )
        self.final_layer.build(input_shape=list(input_shape[:-1]) + [hidden_size])
