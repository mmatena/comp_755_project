...
class EpisodicRetriever(MemoryComponentWithHistory):
    ...
    def build(self, input_shape):
        ...
        self.empty_key = self.add_weight(
            shape=[self.key_size], initializer="random_normal", trainable=True
        )
        self.empty_value = self.add_weight(
            shape=input_shape["inputs"][1:], initializer="random_normal", trainable=True
        )
    ...
    def _retrieve_train(self, inputs, history, history_length, training):
        ...
        print(retrieved_scores_no_grad)
        ignore_mask = tf.cast(
    ...
    def _apply_embeddings(self, values, inputs):
        assert not self.prediction_network.use_position_embeddings
        pos_embeddings = self.prediction_network.pos_embeddings

        values += pos_embeddings
        inputs += pos_embeddings
