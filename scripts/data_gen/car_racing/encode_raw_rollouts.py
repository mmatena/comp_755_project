...
flags.DEFINE_string(
    "model", "raw_rollout_vae_32ld", "Name of model in `saved_models` to use."
)
...
def get_dataset(outer_shard_index, num_outer_shards, num_sub_shards):
    ...
    print(files)
    print(misc.evenly_partition(files, num_outer_shards))
...
def run_shard(
    ...
    filepath = os.path.join(
        ...
        misc.sharded_filename(
            out_name, shard_index=total_shard_index, num_shards=num_total_shards
        ),
    )
